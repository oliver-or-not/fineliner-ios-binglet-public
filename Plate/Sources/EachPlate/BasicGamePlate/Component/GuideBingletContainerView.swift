...
struct GuideBingletContainerView: View {

    // MARK: - Reference

    @Environment(\.colorScheme) var colorScheme
    let viewData: GuideBingletContainerViewData

    // MARK: - Constant

    // MARK: - State

    @State var blinkTrigger: Bool = false
    ...
    var body: some View {
        Spacer()
            ...
            .opacity(blinkTrigger ? 0.3 : 1)
            .animation(Animation.linear(duration: 1.2).repeatForever(autoreverses: true), value: blinkTrigger)
            .onAppear {
                if !blinkTrigger {
                    blinkTrigger = true
                }
...
#Preview {
    GuideBingletContainerView(
        viewData: GuideBingletContainerViewData(
